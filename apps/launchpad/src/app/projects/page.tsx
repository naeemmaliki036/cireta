...
function PlaceholderCover({ title, assetType }: { title: string; assetType: string }) {
  // NOTE: `title` is rendered by the enclosing card via its own <h3> gradient
  // overlay, so we deliberately don't repeat it here — duplicating caused a
  // visible double-title on cards whose banner image failed to load.
  void title;
  ...
  return (
    <div className={`absolute inset-0 bg-gradient-to-br ${gradient}`}>
      ...
      <div className="absolute bottom-4 left-4">
        <p className="text-white/50 text-[10px] font-bold uppercase tracking-[0.2em]">Cireta RWA</p>
      </div>
    </div>
  );
}
