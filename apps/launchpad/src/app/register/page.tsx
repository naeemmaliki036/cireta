...
function RegisterForm() {
  ...
  const { isTermsModalOpen, handleGoogleSignIn, handleTermsAccept, handleTermsClose } = useGoogleOAuth();
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    if (!agreed) { setError("Please read and agree to the Terms of Service and Privacy Policy"); return; }
    setLoading(true);
    setError(null);
    try {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/otp-verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, code: otp, purpose: "register", display_name: displayName }),
        credentials: "include",
      });
      if (!res.ok) {
    ...
    >
      ...
      {step === "info" && (
        <form onSubmit={handleRequestOtp} className="space-y-5">
          <div>
            <label htmlFor="displayName" className="mb-2 block text-sm font-semibold text-text">Full Name</label>
      ...
      {step === "otp" && (
        <form onSubmit={handleVerifyOtp} className="space-y-5">
          <div className="flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={() => { setOtp(""); setError(null); setStep("info"); }}
