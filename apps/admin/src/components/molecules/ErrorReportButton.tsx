...
/**
 * Empathetic error display + Report button. Mirrors the launchpad
 * component so issuer-facing tx failures (deploy, addPhase, activate,
 * etc.) get the same recovery path.
 */
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-5">
            ...
            {done ? (
              ...
              <>
                ...
                <div className="rounded-lg bg-zinc-50 px-3 py-2 mb-3 text-[11px] text-zinc-600 space-y-0.5">
                  {context.functionName && <div><span className="text-zinc-400">Function:</span> {context.functionName}</div>}
                  {context.code && <div><span className="text-zinc-400">Code:</span> {context.code}</div>}
                  {context.txHash && <div className="font-mono break-all"><span className="text-zinc-400 not-italic">Tx:</span> {context.txHash}</div>}
                </div>
                <textarea
                  value={details}
                  onChange={(e) => setDetails(e.target.value)}
                  placeholder="What were you trying to do? Anything that might help us reproduce…"
                  className="w-full px-3 py-2 rounded-lg border border-zinc-200 text-sm focus:outline-none focus:border-darkAqua min-h-[100px]"
                  maxLength={4000}
                />
